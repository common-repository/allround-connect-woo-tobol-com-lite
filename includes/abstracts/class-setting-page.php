...
abstract class Setting_Page implements Action_Hook_Interface {
	...
	public function create_section( $section ) {
		echo '<div class="msn-inside-section">' . esc_html( $section['description'] ) . '</div>';
	}
	...
	function create_text( $args ) {
		...
		echo wc_help_tip( esc_html($args['options']['tooltip']) );
		echo wp_kses( $html, $allowed_html );
	}

	/**
	 * Displays a dropdown field for a settings field
	 *
	 * @param array $args settings field args
	 */
	function create_dropdown( $args ) {
		...
		if (!empty($args['options'])){
			$html = <<< MSNTEXT
				...
				<select class="{$size}-text" name="{$args['section']}[{$args['id']}]" 
				id="{$args['section']}[{$args['id']}]">
			MSNTEXT;

			if ($args['options']['dropdown'][0] == "product_attributes"){
				...
				if ( $attribute_taxonomies ) {
					foreach ($attribute_taxonomies as $tax) {
						if (taxonomy_exists(wc_attribute_taxonomy_name($tax->attribute_name))) {
							array_push($taxonomy_terms, $tax->attribute_name);
						}
					}
				}

				if(count($taxonomy_terms) <= 1){
					$html .= '<option value="'. $taxonomy_terms[0] .'">'. $taxonomy_terms[0] .'</option>';
				} else {
			...
			} elseif ($args['options']['dropdown'][0] == "product_custom_field") {
				$post_type = "product";
				$sample_size = 5;
				$meta_keys = array();
				...
				foreach($meta_keys as $meta){
					$html .= '<option value="'. $meta .'">'. $meta .'</option>';
				}
			} else {
				foreach($args['options']['dropdown'] as $option){
					if ($option == $value){
...
/*		$html  = '<fieldset>';
		...
		$dir = substr(plugin_dir_url( __DIR__ ), 0, -9) . "assets/images/question-mark.svg";
		$tip = wc_help_tip($args['options']['tooltip']);

		if (isset($args['options']['disabled']) && $args['options']['disabled'] == "true"){
			$html = <<< MSNCHECKBOX
					<label for="msnsmp-{$args['section']}[{$args['id']}]">
						<input type="hidden" name="{$args['section']}[{$args['id']}]" value="off" />
						<input type="checkbox" class="checkbox" id="msnsmp-{$args['section']}[{$args['id']}]" name="{$args['section']}[{$args['id']}]" value="on" {$checked_variable} disabled />
						{$args['desc']}
					</label>
		MSNCHECKBOX;
		} else {
	...
	function create_wysiwyg( $args ) {
		...
		echo esc_html( '<div style="max-width: ' . $size . ';">' );
			...
			if ( $sanitize_callback_and_errors['sanitize_callback'] ) {
				...
				if ( $results[ $field_slug ] != $field_value ) {
					...
					if ( $sanitize_callback_and_errors['has_unique_error'] ) {
						...
						$temp_setting_error_args['message'] = __( 'Some values that you entered, were not valid. So they were sanitized and then save into database', ACBOL_Lite_TEXTDOMAIN );
